  ...
  <table>
    ...
    <tbody>
      <tr v-for="(app, index) in processes" :key="index">
        ...
        <td :class="status(app)"><span v-if="games[app.name]">{{ games[app.name].newest }} {{ games[app.name].newestRaw }}</span></td>
        <td :class="status(app)"><span v-if="games[app.name]">{{ games[app.name].lastaccess }} {{ games[app.name].lastaccessRaw }}</span></td>
      </tr>
    </tbody>
  </table>
</template>
